...
def generate_and_save_positions(
    phi_target, xdim, ydim, evanescence, particle_radius, kT, g, B, debye_length,
    max_attempts=100000, output_dir="./", show=False, rng=None
):
    ...
    while len(positions) < N and attempts < max_attempts:
        trial = np.array([
            rng.uniform(0.0, xdim),
            rng.uniform(0.0, ydim),
            sample_z(rng)
        ])
    ...
    if len(positions) < N:
        raise RuntimeError(f"Only placed {len(positions)} out of {N} (attempts={attempts}).")
