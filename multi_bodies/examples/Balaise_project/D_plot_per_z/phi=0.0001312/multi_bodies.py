...
while found_functions is False:
    try:
        ...
        from quaternion_integrator.quaternion_integrator_multi_bodies import (
            QuaternionIntegrator,
        )
        from quaternion_integrator.quaternion_integrator_rollers import (
            QuaternionIntegratorRollers,
        )
...
def calc_slip(bodies, Nblobs, *args, **kwargs):
    ...
    torque_blobs = multi_bodies_functions.calc_one_blob_torques(
        r_vectors, blob_radius=a, g=g
    )
    ...
    if np.amax(np.absolute(torque_blobs)) > 0:
        ...
        if implementation == "pycuda":
            slip_blobs = mb.single_wall_mobility_trans_times_torque_pycuda(
                r_vectors, torque_blobs, eta, a
            )
        elif implementation == "pycuda_no_wall":
            slip_blobs = mb.no_wall_mobility_trans_times_torque_pycuda(
                r_vectors, torque_blobs, eta, a
            )
    ...
    if Laplace_flag is not None:
        ...
        for k, b in enumerate(bodies):
            normals[offset : offset + b.Nblobs] = utils.get_vectors_frame_body(
                b.normals, body=b, translate=False, rotate=True, transpose=False
            )
        ...
        c_background = (
            background_Laplace[0]
            + np.einsum("j,ij->i", background_Laplace[1:4], r_vectors)
            + np.einsum(
                "ik,ik->i", r_vectors, np.einsum("kj,ij->ik", Hessian, r_vectors)
            )
        )

        # Build RHS
        RHS = c_background
        RHS += Laplace_kernels.Laplace_single_layer_operator_numba(
            r_vectors, emitting_rate / diffusion_coefficient, weights, wall=wall
        )

        # Build linear operator
        def linear_operator_Laplace(
            c, r_vectors, normals, weights, reaction_rate, diffusion_coefficient
        ):
            x = 0.5 * c
            x -= Laplace_kernels.Laplace_double_layer_operator_numba(
                r_vectors, c, weights, normals, wall=wall
            )
            x += Laplace_kernels.Laplace_single_layer_operator_numba(
                r_vectors, reaction_rate * c / diffusion_coefficient, weights, wall=wall
            )
            return x

        linear_operator_partial = partial(
            linear_operator_Laplace,
            r_vectors=r_vectors,
            normals=normals,
            weights=weights,
        ...
        A = spla.LinearOperator(
            (Nblobs, Nblobs), matvec=linear_operator_partial, dtype="float64"
        )

        # Call GMRES to get total concentration
        tolerance = read.solver_tolerance
        counter = gmres_counter(print_residual=print_residual)
        (c, info_precond) = utils.gmres(
            A, RHS, tol=tolerance, maxiter=1000, restart=200, callback=counter
        )
        ...
        grad_c += 2 * Laplace_kernels.Laplace_deriv_double_layer_operator_numba(
            r_vectors, c, weights, normals, wall=wall
        ).reshape((Nblobs, 3))
        grad_c -= 2 * Laplace_kernels.Laplace_dipole_operator_numba(
            r_vectors,
            (emitting_rate - reaction_rate * c) / diffusion_coefficient,
            weights,
            wall=wall,
        ).reshape((Nblobs, 3))

        # Compute slip velocity
        slip += surface_mobility[:, None] * (
            grad_c
            - np.einsum("ij,i->ij", normals, np.einsum("ik,ik->i", normals, grad_c))
        )
    ...
    if False:
        ...
        if normals is None:
            ...
            for k, b in enumerate(bodies):
                ...
                weights[offset : offset + b.Nblobs] = b.weights
        # Apply second layer
        Dslip = mb.double_layer_source_target_numba(
            r_vectors, r_vectors, normals, slip, weights, wall=wall
        ).reshape((Nblobs, 3))
        slip = 0.5 * slip + Dslip
...
def set_mobility_vector_prod(implementation, *args, **kwargs):
    ...
    # Implementations different radii
        ...
        return partial(
            mb.mobility_radii_trans_times_force,
...
def K_matrix_vector_prod(bodies, vector, Nblobs, K_bodies=None):
    ...
    for k, b in enumerate(bodies):
        if K_bodies is None:
            ...
            K = K_bodies[k]
        result[offset : offset + b.Nblobs] = np.reshape(
            np.dot(K, v[6 * k : 6 * (k + 1)]), (b.Nblobs, 3)
        )
        offset += b.Nblobs
    return result
...
def C_matrix_T_vector_prod(
    bodies, constraints, vector, Nconstraints, C_constraints=None
):
    """
...
def linear_operator_rigid(
    ...
    **kwargs
    ...
    res[0:Ncomp_blobs] = mobility_vector_prod(
        r_vectors, vector[0:Ncomp_blobs], eta, a, *args, **kwargs
    )
    K_times_U = K_matrix_vector_prod(
        bodies, v[Nblobs : Nblobs + 2 * Nbodies], Nblobs, K_bodies=K_bodies
    )
    res[0:Ncomp_blobs] -= np.reshape(K_times_U, (3 * Nblobs))

    # Compute the "-force_torque" part
    K_T_times_lambda = K_matrix_T_vector_prod(
        bodies, vector[0:Ncomp_blobs], Nblobs, K_bodies=K_bodies
    )
    # Add constraint forces if any
    if Nconstraints > 0:
        C_T_times_phi = C_matrix_T_vector_prod(
            bodies,
            constraints,
            vector[Ncomp_blobs + Ncomp_bodies : Ncomp_tot],
            Nconstraints,
            C_constraints=C_constraints,
        )
        res[Ncomp_blobs : Ncomp_blobs + Ncomp_bodies] = np.reshape(
            -K_T_times_lambda + C_T_times_phi, (Ncomp_bodies)
        )
    else:
        res[Ncomp_blobs : Ncomp_blobs + Ncomp_bodies] = np.reshape(
            -K_T_times_lambda, (Ncomp_bodies)
        )
    ...
    for k, b in enumerate(bodies):
        if b.prescribed_kinematics is True:
            res[3 * offset : 3 * (offset + b.Nblobs)] += (
                K_times_U[offset : (offset + b.Nblobs)]
            ).flatten()
            res[Ncomp_blobs + k * 6 : Ncomp_blobs + (k + 1) * 6] += vector[
                Ncomp_blobs + k * 6 : Ncomp_blobs + (k + 1) * 6
            ]
        offset += b.Nblobs

    # Compute the "constraint velocity: B" part if any
    if Nconstraints > 0:
        C_times_U = C_matrix_vector_prod(
            bodies,
            constraints,
            v[Nblobs : Nblobs + 2 * Nbodies],
            Nconstraints,
            C_constraints=C_constraints,
        )
...
def build_block_diagonal_preconditioners_det_stoch(
    bodies, r_vectors, Nblobs, eta, a, *args, **kwargs
):
    """
    ...
    if (kwargs.get("step") % kwargs.get("update_PC") == 0) or len(
        build_block_diagonal_preconditioners_det_stoch.mobility_bodies
    ) == 0:
        # Loop over bodies
        for k, b in enumerate(bodies):
            if (b.prescribed_kinematics or b.Nblobs == 1) and len(initialized) > 0:
                mobility_bodies.append(
                    build_block_diagonal_preconditioners_det_stoch.mobility_bodies[k]
                )
                K_bodies.append(
                    build_block_diagonal_preconditioners_det_stoch.K_bodies[k]
                )
                M_factorization_blobs.append(
                    build_block_diagonal_preconditioners_det_stoch.M_factorization_blobs[
                        k
                    ]
                )
                M_factorization_blobs_inv.append(
                    build_block_diagonal_preconditioners_det_stoch.M_factorization_blobs_inv[
                        k
                    ]
                )
                mobility_inv_blobs.append(
                    build_block_diagonal_preconditioners_det_stoch.mobility_inv_blobs[k]
                )
            else:
                ...
                M_factorization_blobs_inv.append(
                    scipy.linalg.solve_triangular(
                        L, np.eye(b.Nblobs * 3), check_finite=False
                    )
                )
                ...
                mobility_inv_blobs.append(
                    scipy.linalg.solve_triangular(
                        L,
                        scipy.linalg.solve_triangular(
                            L, np.eye(b.Nblobs * 3), trans="T", check_finite=False
                        ),
                        check_finite=False,
                    )
                )
                ...
                mobility_bodies.append(
                    np.linalg.pinv(
                        np.dot(
                            K.T,
                            scipy.linalg.cho_solve((L, lower), K, check_finite=False),
                        )
                    )
                )
        ...
        build_block_diagonal_preconditioners_det_stoch.mobility_inv_blobs = (
            mobility_inv_blobs
        )
    ...
    def block_diagonal_preconditioner(
        vector,
        bodies=None,
        mobility_bodies=None,
        mobility_inv_blobs=None,
        K_bodies=None,
        Nblobs=None,
        *args,
        **kwargs
        ...
        for k, b in enumerate(bodies):
            if b.prescribed_kinematics is False:
                ...
                result[3 * offset : 3 * (offset + b.Nblobs)] = np.dot(
                    mobility_inv_blobs[k], slip + np.dot(K_bodies[k], Y)
                )
                # 4. Set result
    ...
    block_diagonal_preconditioner_partial = partial(
    ...
    def mobility_pc(
        w, bodies=None, P=None, r_vectors=None, eta=None, a=None, *args, **kwargs
    ):
        ...
        for k, b in enumerate(bodies):
            result[3 * offset : 3 * (offset + b.Nblobs)] = np.dot(
                P[k], w[3 * offset : 3 * (offset + b.Nblobs)]
            )
            offset += b.Nblobs
        # Multiply by M
        ...
        for k, b in enumerate(bodies):
            result[3 * offset : 3 * (offset + b.Nblobs)] = np.dot(
                P[k].T, result_2[3 * offset : 3 * (offset + b.Nblobs)]
            )
            offset += b.Nblobs
        return result

    mobility_pc_partial = partial(
        mobility_pc,
        bodies=bodies,
        P=M_factorization_blobs_inv,
        r_vectors=r_vectors,
        eta=eta,
        a=a,
        *args,
    ...
    def P_inv_mult(w, bodies=None, P_inv=None):
        offset = 0
        for k, b in enumerate(bodies):
            w[3 * offset : 3 * (offset + b.Nblobs)] = np.dot(
                P_inv[k], w[3 * offset : 3 * (offset + b.Nblobs)]
            )
            offset += b.Nblobs
        return w
    ...
    return (
        block_diagonal_preconditioner_partial,
        mobility_pc_partial,
        P_inv_mult_partial,
    )
...
@utils.static_var("M_factorization_blobs_inv", [])
@utils.static_var("mobility_inv_blobs", [])
def build_block_diagonal_preconditioners_det_identity_stoch(
    bodies, r_vectors, Nblobs, eta, a, *args, **kwargs
):
    """
    Build the deterministic and stochastic block diagonal preconditioners for rigid bodies.
    ...
    if (kwargs.get("step") % kwargs.get("update_PC") == 0) or len(
        build_block_diagonal_preconditioners_det_identity_stoch.mobility_bodies
    ) == 0:
        # Loop over bodies
        for b in bodies:
            # 1. Compute blobs mobility
            ...
            mobility_bodies.append(
                np.linalg.pinv(
                    np.dot(
                        K.T, scipy.linalg.cho_solve((L, lower), K, check_finite=False)
                    )
                )
            )
        ...
        build_block_diagonal_preconditioners_det_identity_stoch.mobility_bodies_identity = (
            mobility_bodies_identity
        )
        build_block_diagonal_preconditioners_det_identity_stoch.K_bodies = K_bodies
        build_block_diagonal_preconditioners_det_identity_stoch.M_factorization_blobs = (
            M_factorization_blobs
        )
        build_block_diagonal_preconditioners_det_identity_stoch.M_factorization_blobs_inv = (
            M_factorization_blobs_inv
        )
        build_block_diagonal_preconditioners_det_identity_stoch.mobility_inv_blobs = (
            mobility_inv_blobs
        )
    else:
        # Use old values
    ...
    def block_diagonal_preconditioner_identity(
        vector,
        bodies=None,
        mobility_bodies_identity=None,
        K_bodies=None,
        Nblobs=None,
        *args,
        **kwargs
        ...
        for k, b in enumerate(bodies):
            ...
            F = vector[3 * Nblobs + 6 * k : 3 * Nblobs + 6 * (k + 1)]
            Y = np.dot(
                mobility_bodies_identity[k], -F - np.dot(K_bodies[k].T, Lambda_tilde)
            )
    ...
    block_diagonal_preconditioner_identity_partial = partial(
        block_diagonal_preconditioner_identity,
        bodies=bodies,
    ...
    # Return preconditioner functions
    return (
        block_diagonal_preconditioner_identity_partial,
        mobility_pc_partial,
        P_inv_mult_partial,
    )
...
@utils.static_var("mobility_inv_blobs", [])
def build_block_diagonal_preconditioner(
    bodies, articulated, r_vectors, Nblobs, eta, a, *args, **kwargs
):
    """
    ...
    res_art_bodies = []
    if (kwargs.get("step") % kwargs.get("update_PC") == 0) or len(
        build_block_diagonal_preconditioner.mobility_bodies
    ) == 0:
        # loop over bodies
        for k, b in enumerate(bodies):
            if (b.prescribed_kinematics or b.Nblobs == 1) and len(initialized) > 0:
                mobility_inv_blobs.append(
                    build_block_diagonal_preconditioner.mobility_inv_blobs[k]
                )
                mobility_bodies.append(
                    build_block_diagonal_preconditioner.mobility_bodies[k]
                )
        ...
        for ka, art in enumerate(articulated):
            ...
            for kc, const in enumerate(art.constraints):
                ...
                C1 = C_art_bodies[ka][
                    3 * kc : 3 * (kc + 1), 6 * b1loc : 6 * (b1loc + 1)
                ]
                C2 = C_art_bodies[ka][
                    3 * kc : 3 * (kc + 1), 6 * b2loc : 6 * (b2loc + 1)
                ]
                CN[3 * kc : 3 * (kc + 1), 6 * b1loc : 6 * (b1loc + 1)] = np.dot(
                    C1, mobility_bodies[b1]
                )
                CN[3 * kc : 3 * (kc + 1), 6 * b2loc : 6 * (b2loc + 1)] = np.dot(
                    C2, mobility_bodies[b2]
                )
    ...
    def block_diagonal_preconditioner(
        ...
        for k, b in enumerate(bodies):
            if b.prescribed_kinematics is False:
                ...
                F = vector[3 * Nblobs + 6 * k : 3 * Nblobs + 6 * (k + 1)]
                U_unconst = np.dot(
                    mobility_bodies[k], -F - np.dot(K_bodies[k].T, Lambda_tilde)
                )

                # 3. solve M*Lambda = (slip + K*y)
                Lambda = np.dot(
                    mobility_inv_blobs[k], slip + np.dot(K_bodies[k], U_unconst)
                )
        ...
        for ka, art in enumerate(articulated):
            ...
            # C*U_unconst
            C_times_U = np.dot(
                C_art_bodies[ka], U_unconst[6 * indb_first : 6 * (indb_last + 1)]
            )
            ...
            for kb, b in enumerate(art.bodies):
                ...
                NFc[6 * kb : 6 * (kb + 1)] = np.dot(
                    mobility_bodies[indb], Fc[6 * kb : 6 * (kb + 1)]
                )
                # Computes the correction for lambda due to constraints
                Lambda_corr = np.dot(
                    mobility_inv_blobs[indb],
                    np.dot(K_bodies[indb], NFc[6 * kb : 6 * (kb + 1)]),
                )
            ...
            result[
                3 * Nblobs + 6 * indb_first : 3 * Nblobs + 6 * (indb_last + 1)
            ] += NFc
            # Set result Phi
            result[
                3 * Nblobs
...
@utils.static_var("k_bodies", [])
@utils.static_var("mobility_inv_blobs", [])
def build_block_diagonal_preconditioner_articulated_identity(
    bodies, constraints, articulated, r_vectors, Nblobs, eta, a, *args, **kwargs
):
    """
    build the block diagonal preconditioner for articulated rigid bodies.
    it solves exactly the mobility problem for each body
    ...
    def block_diagonal_preconditioner(
        ...
        result[3 * Nblobs + 6 * len(bodies) : Ncomp] = vector[
            3 * Nblobs + 6 * len(bodies) : Ncomp
        ]
        return result
...
@utils.static_var("C_art_bodies", [])
@utils.static_var("res_art_bodies", [])
def build_block_diagonal_preconditioner_articulated_single_blobs(
    bodies, articulated, Nblobs, Nconstraints, eta, a, *args, **kwargs
):
    """
    build the block diagonal preconditioner for articulated rigid bodies.
    it solves exactly the mobility problem for each body
    ...
    if (kwargs.get("step") % kwargs.get("update_PC") == 0) or len(
        ...
        factor_Mtt = 1.0 / (6 * np.pi * eta * a)
        factor_Mrr = 1.0 / (8 * np.pi * eta * a**3)
        M = np.eye(6)
    ...
    def block_diagonal_preconditioner(
        ...
        result[3 * Nconstraints : 3 * Nconstraints + 6 * Nblobs] = vector[
            0 : 6 * Nblobs
        ]
...
if __name__ == "__main__":
    ...
    multi_bodies_functions.calc_body_body_forces_torques = (
        multi_bodies_functions.set_body_body_forces_torques(
    ...
    for ID, structure in enumerate(structures):
        ...
        num_bodies_struct, struct_locations, struct_orientations = (
            read_clones_file.read_clones_file(structure[1])
        )
    ...
    for ID, structure in enumerate(read.articulated):
        ...
        struct_ref_config = read_vertex_file_list.read_vertex_file_list(
            structure[0], output_name
        )
        num_bodies_struct, struct_locations, struct_orientations = (
            read_clones_file.read_clones_file(structure[1])
        )
        constraints_info = read_constraints_file.read_constraints_file(
            structure[2], output_name
        )
        ...
        for i in range(num_bodies_struct):
            ...
            b = body.Body(
                struct_locations[i],
                struct_orientations[i],
                struct_ref_config[subbody],
                a,
            )
            b.mobility_blobs = set_mobility_blobs(read.mobility_blobs_implementation)
        ...
        num_constraints_total = num_constraints * (
            num_bodies_struct // num_bodies_in_articulated
        )
        ...
        for i in range(num_constraints_total):
            ...
            bodies_indices = (
                constraints_bodies[subconstraint]
                + num_bodies_in_articulated * articulated_body
                + bodies_offset
            )
            ...
            c = Constraint(
                ...
                articulated_body,
                parameters,
                constraints_extra[subconstraint],
            )
        ...
        for i in range(num_bodies_struct // num_bodies_in_articulated):
            bodies_indices = (
                bodies_offset
                + i * num_bodies_in_articulated
                + np.arange(num_bodies_in_articulated, dtype=int)
            )
            bodies_in_articulated = bodies[bodies_indices[0] : bodies_indices[-1] + 1]
            constraints_indices = (
                constraints_offset
                + i * num_constraints
                + np.arange(num_constraints, dtype=int)
            )
            constraints_in_articulated = constraints[
                constraints_indices[0] : constraints_indices[-1] + 1
            ]
            art = Articulated(
                bodies_in_articulated,
                bodies_indices,
    ...
    if scheme.find("rollers") == -1:
        integrator = QuaternionIntegrator(
            bodies, Nblobs, scheme, tolerance=read.solver_tolerance, domain=read.domain
        )
        integrator.build_block_diagonal_preconditioner = (
            build_block_diagonal_preconditioner
        )
        integrator.first_guess = np.zeros(
            Nblobs * 3 + num_bodies * 6 + len(constraints) * 3
        )
    else:
        ...
        integrator.calc_blob_blob_forces = partial(
            multi_bodies_functions.set_blob_blob_forces(
        ...
        integrator.build_block_diagonal_preconditioner = (
            build_block_diagonal_preconditioner_articulated_single_blobs
        )
        ...
        integrator.shear = read.shear
    integrator.n_save = n_save
    integrator.calc_slip = partial(
        calc_slip,
    ...
    integrator.mobility_vector_prod = set_mobility_vector_prod(
        read.mobility_vector_prod_implementation, bodies=bodies
    )
    mobility_vector_prod = set_mobility_vector_prod(
        read.mobility_vector_prod_implementation, bodies=bodies
    )
    integrator.force_torque_calculator = partial(
        multi_bodies_functions.force_torque_calculator_sort_by_bodies,
        g=g,
        repulsion_strength_wall=read.repulsion_strength_wall,
        debye_length_wall=read.debye_length_wall,
        repulsion_strength=read.repulsion_strength,
    ...
    integrator.build_block_diagonal_preconditioners_det_stoch = (
        build_block_diagonal_preconditioners_det_stoch
    )
    integrator.build_block_diagonal_preconditioners_det_identity_stoch = (
        build_block_diagonal_preconditioners_det_identity_stoch
    )
    integrator.eta = eta
    integrator.a = a
    ...
    multi_bodies_functions.calc_blob_blob_forces = (
        multi_bodies_functions.set_blob_blob_forces(
            read.blob_blob_force_implementation, bodies=bodies
        )
    )
    integrator.plot_velocity_field = read.plot_velocity_field
    integrator.output_name = read.output_name
    integrator.mobility_vector_prod_implementation = (
        read.mobility_vector_prod_implementation
    )
    try:
        integrator.plot_velocity_field_shell = (
            multi_bodies_functions.plot_velocity_field_shell
        )
    except:
        pass
    ...
    for step in range(read.initial_step, n_steps):
        ...
        if (step % n_save) == 0 and step >= 0:
            ...
            print(
                ...
                integrator.invalid_configuration_count,
                ", wallclock time = ",
            ...
            if read.save_blobs_mobility == "True" or read.save_body_mobility == "True":
                ...
                mobility_blobs = integrator.mobility_blobs(
                    r_vectors_blobs, read.eta, read.blob_radius
                )
                if read.save_blobs_mobility == "True":
                    name = (
                        output_name + ".blobs_mobility." + str(step).zfill(8) + ".dat"
                    )
                ...
                if read.save_body_mobility == "True":
                    ...
                    mobility_bodies = np.linalg.pinv(
                        np.dot(K.T, np.dot(resistance_blobs, K))
                    )
        ...
        # Update HydroGrid
        if (
            (step % read.sample_HydroGrid) == 0
            and found_HydroGrid
            and read.call_HydroGrid
        ):
            cc.calculate_concentration(
                output_name,
                read.periodic_length[0],
    ...
    if ((step + 1) % n_save) == 0 and step >= 0:
        ...
        if read.save_blobs_mobility == "True" or read.save_body_mobility == "True":
            ...
            if read.save_blobs_mobility == "True":
                name = (
                    output_name + ".blobs_mobility." + str(step + 1).zfill(8) + ".dat"
                )
                np.savetxt(name, mobility_blobs, delimiter="  ")
            if read.save_body_mobility == "True":
                resistance_blobs = np.linalg.inv(mobility_blobs)
    ...
    if (
        ((step + 1) % read.sample_HydroGrid) == 0
        and found_HydroGrid
        and read.call_HydroGrid
    ):
        cc.calculate_concentration(
    ...
    with open(output_name + ".info", "w") as f:
        ...
        f.write(
            ...
            + str(integrator.invalid_configuration_count)
            + "\n"
            + "deterministic_iterations_count = "
            + str(integrator.det_iterations_count)
            + "\n"
            + "stochastic_iterations_count    = "
            + str(integrator.stoch_iterations_count)
            + "\n"
            + "nonlinear_iterations_count     = "
            + str(nonlinear_counter)
            + "\n"
        )
    print("\n\n\n# End")
