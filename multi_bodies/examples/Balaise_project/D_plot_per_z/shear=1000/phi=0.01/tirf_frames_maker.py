...
def compute_bounds(frames: List[np.ndarray], pad=0.0):
...
def main():
    ap = argparse.ArgumentParser(description="Render 2D particle positions (z = color) to image frames.")
    ...
    ap.add_argument(
        "--size", type=int, nargs=2, metavar=("W", "H"),
        default=(512, 512),
        help="Figure size in pixels."
    )
    ap.add_argument("--marker-size", type=float, default=5.0, help="Marker size.")
    args = ap.parse_args()

    frames, n = parse_config(args.input)
    n_frames = len(frames)
    (xmin, xmax), (ymin, ymax), (zmin, zmax) = compute_bounds(frames, pad = 0.0)
    ...
    # ax.set_xlabel("X", color="white") # No more legend or axes
    ...
    order0 = np.argsort(visible0[:, 2])[::-1]
    visible0 = visible0[order0]

    scat = ax.scatter(
        visible0[:, 0], visible0[:, 1],
        c=visible0[:, 2],
        cmap=cmap,
        s=args.marker_size,
        vmin=0,
    ...
    for i, pts in enumerate(frames):
        ...
        if np.any(mask):
            visible = pts[mask]
            order = np.argsort(visible[:, 2])[::-1]
            visible = visible[order]

            scat.set_offsets(visible[:, :2])
            scat.set_array(visible[:, 2])
        else:
...
# python3 tirf_frames_maker.py run_blobs.sphere_array.config --outdir tirf_frames
